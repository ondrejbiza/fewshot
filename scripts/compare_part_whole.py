...
def pybullet_load_object(cfg, pc_master_dict, upright_orientation_dict, pb_client, table_id, table_base_id, new_parent_scale):
    poses = {}
    for pc in ['child', 'parent']:
        ...
        if load_pose_type == 'any_pose':
            ...
            if load_pose_type == 'demo_pose':
                ...
                pos = [np.random.random() * (x_high - x_low) + x_low, np.random.random() * (y_high - y_low) + y_low, cfg.TABLE_Z]
...
def initialize_pointcloud_data(cfg, parent_class, child_class, demos, is_parent_shapenet_obj, is_child_shapenet_obj, parent_load_pose_type, child_load_pose_type):
...
def main(args):
    ...
    pb_client = create_pybullet_client(gui=False, opengl_render=True, realtime=False, server=args.pybullet_server)#args.pybullet_viz
    ...
    pc_master_dict = initialize_pointcloud_data(cfg, parent_class, child_class, demos, is_parent_shapenet_obj, is_child_shapenet_obj, args.parent_load_pose_type, args.child_load_pose_type)
    ...
    for pc in ['child', 'parent']:
        for i in range(len(pc_master_dict[pc]['demo_start_pcds'])):
            ...
            if pc_master_dict[pc]['class'] == 'mug':
                ...
                adjusted_demo_parts = {source_part_names[0]: pc_master_dict[pc]['demo_start_pcds'][i]}
    ...
    canon_source_parts_paths = {part:f"part_based_warp_models/{part}_{args.canon_source_file_stamp}" for part in source_part_names}
    canon_target_parts_paths = {part:f"part_based_warp_models/{part}_{args.canon_target_file_stamp}" for part in target_part_names}

    canon_source_parts = {part: pickle.load(open(canon_source_parts_paths[part], 'rb')) for part in source_part_names}
    canon_target_parts = {part: pickle.load(open(canon_target_parts_paths[part], 'rb')) for part in target_part_names}

    canon_source_training_ids = canon_source_parts[source_part_names[0]].metadata.training_ids + [canon_source_parts[source_part_names[0]].metadata.canonical_id]
    canon_target_training_ids = canon_target_parts[target_part_names[0]].metadata.training_ids + [canon_target_parts[target_part_names[0]].metadata.canonical_id]
    ...
    canon_source_whole_path = {part:f"part_based_warp_models/{part}_{args.canon_source_file_stamp}" for part in source_whole_name}
    canon_target_whole_path = {part:f"part_based_warp_models/{part}_{args.canon_target_file_stamp}" for part in target_whole_name}
    ...
    for interface, exp_type in zip([part_interface, whole_interface], ['parts_based', 'whole']):
        #####################################################################################
        # set the demo info
        max_demos = len(demos)
        ...
        if args.demo_selection:
            ...
            for i in range(n_demos):
                try: 
                    cost, program = interface.set_demo_info(pc_master_dict, demo_idx=i, calculate_cost=True, show=True)
                    costs.append(cost)
        ...
        #####################################################################################
            ...
            if args.test_on_train:
                ...
                #         break
                if args.sweep:
                    child_objects = (pc_master_dict['child']['test_ids'] + pc_master_dict['child']['train_ids']) - canon_source_training_ids
                    parent_objects = (pc_master_dict['parent']['test_ids'] + pc_master_dict['parent']['train_ids']) - canon_target_training_ids
                    child_id = random.sample(child_objects, 1)[0]
                    while True: 
                        parent_id = random.sample(parent_objects, 1)[0]
            ...
            poses = pybullet_load_object(cfg, pc_master_dict, upright_orientation_dict, pb_client, table_id, table_base_id, new_parent_scale)
            ...
            actual_parent_pose = np.concatenate(pb_client.get_body_state(parent_obj_id)[:2]).tolist()
            print(actual_parent_pose)
            parent_parts, _, start_parent_transforms = segment_mug(parent_id, utils.pos_quat_to_transform(actual_parent_pose[:3], actual_parent_pose[3:]))#segment_mug_rack(parent_pcd)
            ...
            if exp_type == "parts_based":
                ...
                relative_trans = interface.infer_relpose(child_pcd, parent_pcd, None, se3=se3, experiment_id=experiment_id)
            pause_mc_thread(False)
            time.sleep(1.0)
            ...
            eval_iter_dir = osp.join(eval_save_dir, f'trial_{iteration}')
            util.safe_makedirs(eval_iter_dir)
            sample_fname = osp.join(eval_iter_dir, f'{exp_type}_success_rate_relation.npz')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sweep', type=bool, default=False, help="If true tests on all valid object ids, not just those in test_ids. Shouldn't run with rndf")
