...
def main(args):
    ...
    robotiq = trimesh.load("data/main_robotiq.stl")
    robotiq_vertices = np.array(robotiq.vertices)
    robotiq_faces = np.array(robotiq.faces)
    ...
    def display_mesh(axis1, axis2, axis3, axis4):
        ...
        robotiq_color = np.zeros((len(robotiq_faces), 3), dtype=np.float32)
        robotiq_color[:, 0] = np.linspace(0.0, 0.5, len(robotiq_color))
        robotiq_color[:, 1] = np.linspace(0.0, 1.0, len(robotiq_color))

        trans, _, _ = utils.best_fit_transform(pos_robotiq, pcd[grasp_index])
        tmp_robotiq_vertices = utils.transform_pcd(robotiq_vertices, trans)
        ...
        data = [
            ...
            # go.Mesh3d(
            #     x=tmp_robotiq_vertices[:, 0], y=tmp_robotiq_vertices[:, 1], z=tmp_robotiq_vertices[:, 2], 
            #     i=robotiq.faces[:, 0], j=robotiq.faces[:, 1], k=robotiq.faces[:, 2],
            #     facecolor=robotiq_color
            # ),
            make_sphere(pcd[grasp_index[0]], 0.01),
            # make_sphere(pcd[grasp_index[1]], 0.01),
            # make_sphere(pcd[grasp_index[2]], 0.01),
            # make_sphere(pcd[grasp_index[3]], 0.01),
            # make_sphere(pcd[grasp_index[4]], 0.01),
        ]
