...
def main(args):
    ...
    def display_mesh(axis1, axis2, axis3, axis4):
        ...
        robotiq_color = np.zeros((len(robotiq.faces), 3), dtype=np.float32)
        ...
        data = [
            go.Mesh3d(
                ...
                facecolor=color),
            go.Mesh3d(
                x=robotiq.vertices[:, 0], y=robotiq.vertices[:, 1], z=robotiq.vertices[:, 2], 
            ...
            # Intensity of each vertex, which will be interpolated and color-coded
            # intensity = np.linspace(0, 1, len(vertices)),
            make_sphere(pcd[rim_index], 0.01),
        ]
