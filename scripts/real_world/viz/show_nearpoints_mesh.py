...
def main(args):
    ...
    def display_mesh(axis1, axis2, axis3, axis4):
        ...
        x = np.array([0.02, 0.053, 0.029])
        d = np.square(targets - x[None]).sum(axis=1)
        i = np.argmin(d)
        ...
        data = [
            ...
            make_sphere(targets[i], 0.01),
            # *[make_sphere(anchors[i, j], 0.01) for j in range(anchors.shape[1])],
            # *[make_sphere(targets[i], 0.01) for i in range(min(50, len(targets)))]
        ]
...
parser.add_argument("load_path", default="data/230330/mug_tree_ndf_place_4.pkl")
