...
def main(args):
    ...
    assert args.relation_method in [
        "intersection",
        "ebm",
    ], "Invalid argument for --relation_method"
    ...
    modelstr = (
        f"parent_model--{parent_model_save_path}_child_model--{child_model_save_path}"
    )
    ...
    parent_model_path = osp.join(
        path_util.get_rndf_model_weights(), args.parent_model_path
    )
    child_model_path = osp.join(
        path_util.get_rndf_model_weights(), args.child_model_path
    )

    if args.parent_model_path_ebm is not None:
        parent_model_path_ebm = osp.join(
            path_util.get_rndf_model_weights(), args.parent_model_path_ebm
        )
    else:
        parent_model_path_ebm = parent_model_path
    if args.child_model_path_ebm is not None:
        child_model_path_ebm = osp.join(
            path_util.get_rndf_model_weights(), args.child_model_path_ebm
        )
    else:
        child_model_path_ebm = child_model_path

    parent_model = vnn_occupancy_network.VNNOccNet(
        latent_dim=256, model_type="pointnet", return_features=True, sigmoid=True
    ).cuda()
    child_model = vnn_occupancy_network.VNNOccNet(
        latent_dim=256, model_type="pointnet", return_features=True, sigmoid=True
    ).cuda()
    ...
    assert (
        args.parent_load_pose_type in valid_load_pose_types
    ), f'Invalid string value for args.parent_load_pose_type! Must be in {", ".join(valid_load_pose_types)}'
    assert (
        args.child_load_pose_type in valid_load_pose_types
    ...
    target_desc_subdir = create_target_desc_subdir(
        demo_path, parent_model_path, child_model_path
    )
    target_desc_fname = osp.join(target_desc_subdir, args.target_desc_name)
    ...
    if args.relation_method == "intersection":
        if not osp.exists(target_desc_fname) or args.new_descriptors:
            ...
            if n_demos != "all":
                pc_master_dict["parent"]["demo_final_pcds"] = pc_master_dict["parent"][
                    "demo_final_pcds"
                ][args.demo_idx : args.demo_idx + n_demos]
                pc_master_dict["child"]["demo_final_pcds"] = pc_master_dict["child"][
                    "demo_final_pcds"
                ][args.demo_idx : args.demo_idx + n_demos]
            ...
            create_target_descriptors(
                ...
                pc_reference=args.pc_reference,
                skip_alignment=args.skip_alignment,
                n_demos="all",
                manual_target_idx=args.target_idx,
                add_noise=add_noise,
                interaction_pt_noise_std=noise_value,
                use_keypoint_offset=use_keypoint_offset,
                keypoint_offset_params=keypoint_offset_params,
                visualize=False,
            )
    ...
    if osp.exists(target_desc_fname):
        ...
        parent_overall_target_desc = target_descriptors_data[
            "parent_overall_target_desc"
        ]
        child_overall_target_desc = target_descriptors_data["child_overall_target_desc"]
        parent_overall_target_desc = (
            torch.from_numpy(parent_overall_target_desc).float().cuda()
        )
        child_overall_target_desc = (
            torch.from_numpy(child_overall_target_desc).float().cuda()
        )
    ...
    if args.relation_method == "ebm" or args.refine_with_ebm:
        rel_model = EBM().cuda()
        rel_model_path = osp.join(
            path_util.get_rndf_model_weights(),
            "relation_energy/cachedir",
            args.rel_model_path,
        )
        assert osp.exists(
            rel_model_path
        ), f"Path to relation energy model: {rel_model_path} does not exist!"
    ...
    if args.sweep:
        if args.exp == "mug_on_rack_sweep":
            child_objects = pc_master_dict["child"]["test_ids"]
    ...
    for iteration in range(args.start_iteration, args.num_iterations):
        ...
        if is_parent_shapenet_obj:
            parent_obj_file = osp.join(
                mesh_data_dirs[parent_class], parent_id, "models/model_normalized.obj"
            )
        ...
        if parent_class == "syn_container" and child_class == "bottle":
            ...
            container_upright_mat[:-1, :-1] = common.quat2rot(
                container_upright_orientation
            )
            ...
            container_flat = np.hstack(
                [container_2d, np.zeros(container_2d.shape[0]).reshape(-1, 1)]
            )
            bottle_flat = np.hstack(
                [bottle_2d, np.zeros(bottle_2d.shape[0]).reshape(-1, 1)]
            )
            ...
            if np.max(bottle_extents) > (0.75 * np.min(container_extents[:-1])):
                ...
                new_parent_scale = (
                    np.max(bottle_extents)
                    * (np.random.random() * (2 - 1.5) + 1.5)
                    / np.min(container_extents[:-1])
                )
        ...
        for pc in pcl:
            ...
            if args.rand_mesh_scale:
                mesh_scale = [
                    np.random.random() * (scale_high - scale_low) + scale_low
                ] * 3
            else:
            ...
            if load_pose_type == "any_pose":
                ...
                if load_pose_type == "demo_pose":
                    obj_start_pose_demo = pc_master_dict[pc]["demo_start_poses"][
                        demo_idx
                    ]
        ...
        if parent_class == "syn_container" and child_class == "bottle":
            bottle_final_pose = np.concatenate(
                p.getBasePositionAndOrientation(pc_master_dict["child"]["pb_obj_id"])[
                    :2
                ]
            ).tolist()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no_trans",
        action="store_true",
        help="whether or not to include translation opt",
    )
    parser.add_argument(
        "--load_start",
        action="store_true",
        help="if we should load the start point clouds from demos",
    )
