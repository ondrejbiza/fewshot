...
class ObjectWarpingSE3Batch:
    ...
    def inference(self, initial_poses: NDArray[np.float32]
        ...
        self.latent_param = nn.Parameter(
            torch.zeros((n_angles, self.pca.n_components), dtype=torch.float32, device=self.device),
            requires_grad=True)
        self.center_param = nn.Parameter(
            torch.zeros((n_angles, 3), dtype=torch.float32, device=self.device),
            requires_grad=True)
...
class ObjectWarpingSE2Batch(ObjectWarpingSE3Batch):
...
class ObjectSE3Batch:
    ...
    def inference(self, initial_poses: NDArray[np.float32]
        ...
        for _ in range(self.n_steps):
            ...
            if self.n_samples is not None:
                canonical_pcd_ = self.subsample(self.n_samples)
            else:
                canonical_pcd_ = self.canonical_pcd

            self.optim.zero_grad()
            new_pcd = self.create_transformed_pcd(canonical_pcd_)
            cost = cost_batch_pt(self.pcd[None], new_pcd)
        ...
        with torch.no_grad():
            # Compute final cost without subsampling.
            new_pcd = self.create_transformed_pcd(self.canonical_pcd)
            cost = cost_batch_pt(self.pcd[None], new_pcd)
    ...
    def subsample(self, num_samples: int) -> torch.Tensor:
        """Randomly subsample the canonical object, including its PCA projection."""
        indices = np.random.randint(0, self.canonical_pcd.shape[0], num_samples)
        return self.canonical_pcd[indices]
...
class ObjectSE2Batch(ObjectSE3Batch):
...
def warp_to_pcd_se3(object_warping: Union[ObjectWarpingSE3Batch, ObjectSE3Batch], n_angles: int=50,
    ...
    poses = random_ortho_rots(n_angles * n_batches)
...
def random_ortho_rots_hemisphere(num: int) -> NDArray[np.float32]:
    """Sample random initial 2x3 rotation matrices.
    
    Matrices that flip the object upside down are rejected.
    """
...
def random_ortho_rots(num: int) -> NDArray[np.float32]:
    """Sample random initial 2x3 rotation matrices."""
    v1 = np.random.normal(0, 1, (num, 3)).astype(np.float32)
    v2 = np.random.normal(0, 1, (num, 3)).astype(np.float32)
