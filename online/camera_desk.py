...
import tf2_ros
import tf
from online import transformation
import time
...
class CloudProxyDesk:
    def __init__(self, desk_center=[-0.527, -0.005], z_min=-0.07, enable_cam3=True, obs_clip_offset=0, desk_offset=0.05):
        ...
        self.obs_clip_offset = obs_clip_offset
        self.desk_offset = desk_offset
    ...
    def getNewPointCloud(self, gripper_pos):
        ...
        if self.enable_cam3:
            ...
            cloud = np.concatenate((cloud1, cloud2))
        # filter workspace
        ...
        # cloud = cloud[(cloud[:, 2] < max(gripper_pos[2]+self.obs_clip_offset, self.z_min + 0.05))]
        cloud = cloud[(cloud[:, 2] < max(gripper_pos[2] + self.obs_clip_offset, self.z_min + self.desk_offset))]
        # filter ws x
...
def main():
    ...
    while True:
        ...
        obs = cloudProxy.getProjectImg(0.3, 128, (-0.525, 0, 0.13))
        # obs = -obs
        # obs -= obs.min()
